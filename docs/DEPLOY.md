...
There are a few images provided: the `actionloop-base` and the `action-golang-v1.17` and `action-golang-v1.18` are available. Each image accepts different input in the deployment.
